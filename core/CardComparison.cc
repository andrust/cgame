...
int compare_rank::operator()(const Card& lhs, const Card& rhs) {
    ...
    switch(_dir) {
        ...
        case eOrderDir::SUITLESS_ASC:
            if(lhs.GetRank() == eRanks::X) {
                return rhs.GetRank() >= eRanks::LOW ? 1 : -1;
            }
            else if(rhs.GetRank() == eRanks::X) {
                return lhs.GetRank() <= eRanks::IX ? 1 : -1;
            }
            else {
                return lhs.GetRank() > rhs.GetRank() ? 1 : -1;
            }
        case eOrderDir::SUITLESS_DESC:
            if(lhs.GetRank() == eRanks::X) {
                return rhs.GetRank() <= eRanks::IX ? 1 : -1;
            }
            else if(rhs.GetRank() == eRanks::X) {
                return lhs.GetRank() >= eRanks::LOW ? 1 : -1;
            }
            else {
                return lhs.GetRank() < rhs.GetRank() ? 1 : -1;
            }
